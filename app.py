...
#         # Predict from model
#         # Save video as .mp4 instead of .npy for inference.py
#         video_path = save_path  # already saved in uploads
#         subtitle = predict_from_video(video_path)
...
MODEL_PATH = "D://LipReadingProject//New_model_code//LipCoordNet//LipCoordNet_Project//LipCoordNet//lip_coordinate_extraction//shape_predictor_68_face_landmarks_GTX.dat"
...
# Initialize dlib face detector (not required in this file but may be reused)
detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor(MODEL_PATH)
...
def ajax_upload():
    ...
    if file and allowed_file(file.filename):
        ...
        # Run inference using the pretrained LipCoordNet model
        subtitle = predict_from_video(save_path)
        print(f"[✔] Subtitle output: {subtitle}")
